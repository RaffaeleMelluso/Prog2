...
public  class Rubrica { 
    ...
    private int compara(String n)
    {
        int i=0;
        int pos=-1;
        while(i<numContatti && pos==-1)
        {
            
            if(n.compareTo(contatti[i].getNome())<0)
            {
                pos=i;
            }
            i++;
    ...
    private Contatto[] estendi()
    {
        Contatto[] contatton=new Contatto[2*numContatti];
        for(int i=0;i<numContatti;i++)
        {
            contatton[i]=contatti[i];
        }
        return contatton;
    }
